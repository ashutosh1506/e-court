export default function Home() {
  ...
  return (
    <section className="bg-base-200 py-12 flex flex-col items-center">
      <h2 className="text-3xl font-bold text-primary mb-8">Notices & Updates</h2>
      <div className="w-full max-w-3xl flex flex-col gap-6">
        {notices.map((n, i) => (
          <div key={i} className="card bg-white shadow-lg border-l-4 border-warning p-6 relative">
            <div className="flex items-center gap-3 mb-2">
              <span className="text-sm text-gray-500">{n.date}</span>
              <span className="badge badge-warning text-black">{n.category}</span>
            </div>
            <a
              href={n.link}
